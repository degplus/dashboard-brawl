...
# ============================================================
# IMAGE → BASE64 (used for Players table only)
# ============================================================
...
# ============================================================
# GET FILTER OPTIONS  (FIXED for f_player masking)
# ============================================================
def get_filter_options(exclude_key: str) -> list:
    ...
    # Apply all other selected filters to the mask
    for key, col in COL_MAP.items():
        ...
        # IMPORTANT: f_player stores DISPLAY NAMES in session_state, but df_dim has player_tag
        if key == "f_player":
    ...
    # Options for the current dropdown (exclude_key)
    col = COL_MAP[exclude_key]
    raw = sorted(df_dim[mask][col].dropna().unique().tolist())

    # For the Player dropdown, show display names (not tags)
...
# ============================================================
# BUILD WHERE — general filters (BigQuery)
# ============================================================
...
# ============================================================
# BUILD WHERE H2H — excludes f_team
# ============================================================
def build_where_h2h():
...
if df_brawlers.empty:
    ...
    st.caption("Brawlers with Pick Rate ≥ 30% but Win Rate < 45% — high trust, low delivery. Consider avoiding in draft.")
...
with st.expander("🤝 Duo Analysis", expanded=False):
    tab_syn, tab_ctr = st.tabs(["⚡ Synergy — Same Team", "🛡️ Counter — Opposite Teams"])

    with tab_syn:
        st.caption(
            "Pairs of brawlers that played on the **same team** in the same match. "
            "Minimum 10 games. Sorted by win rate."
        )
        ...
        if df_syn.empty:
            st.info("Not enough data to show synergy pairs (minimum 10 games per pair).")
        else:
            st.dataframe(
                df_syn,
                use_container_width=True,
                column_order=["img_a", "img_b", "games", "win_rate"],
                column_config={
                    "img_a": st.column_config.ImageColumn("Brawler A"),
                    "img_b": st.column_config.ImageColumn("Brawler B"),
                    "games": st.column_config.NumberColumn("Games", format="%d"),
                    "win_rate": st.column_config.ProgressColumn("Win Rate", format="%.1f%%", min_value=0, max_value=100),
                },
                hide_index=True
    ...
    with tab_ctr:
        st.caption(
            ...
            "Win Rate shows how often Brawler A's team won. Minimum 10 games."
        )
        ...
        if df_ctr.empty:
            ...
            st.dataframe(
                df_ctr,
                use_container_width=True,
                column_order=["img_a", "img_b", "games", "win_rate_a"],
                column_config={
                    "img_a": st.column_config.ImageColumn("Brawler A"),
                    "img_b": st.column_config.ImageColumn("Brawler B"),
                    "games": st.column_config.NumberColumn("Games", format="%d"),
...
df_teams = fetch_data(f"""
    ...
    brawler_picks AS (
        SELECT player_team, brawler_name,
               COUNT(*) AS picks,
...
df_games = fetch_data(f"""
    ...
    FROM `brawl-sandbox.brawl_stats.vw_battles_python`
    WHERE {where_main}
    GROUP BY game, battle_time, map, mode
...
if df_games.empty:
    ...
    ev = st.dataframe(
        df_games,
        use_container_width=True,
        column_order=["game", "battle_time", "map", "mode"],
        column_config={
    ...
    df_h2h = fetch_data(f"""
        WITH my_games AS (
            SELECT DISTINCT game,
                   player_team   AS my_team,
                   player_result AS my_result
            FROM `brawl-sandbox.brawl_stats.vw_battles_python`
            WHERE {where_h2h} AND player_team = @h2h_team
        ),
        opponent_games AS (
            SELECT DISTINCT game,
                   player_team   AS opp_team,
                   player_result AS opp_result
            FROM `brawl-sandbox.brawl_stats.vw_battles_python`
    ...
    if df_h2h.empty:
        ...
        if sel_opp:
            ...
            df_matchups = fetch_data(f"""
                ...
                opp_games AS (
                    SELECT DISTINCT game,
                           ...
                           player_result AS opp_result
                    FROM `brawl-sandbox.brawl_stats.vw_battles_python`
                    WHERE player_team = @opp_name
                ),
                joined AS (
                    SELECT mg.game, mg.my_team, mg.my_result,
                           og.opp_team, og.opp_result
                    FROM my_games mg
                    JOIN opp_games og ON mg.game = og.game
                )
                SELECT
                    j.game,
                    FORMAT_TIMESTAMP('%Y-%m-%d %H:%M', v.battle_time) AS battle_time,
                    MAX(v.map)     AS map,
                ...
                GROUP BY j.game, v.battle_time
                ORDER BY v.battle_time DESC
            """, params_matchup)
            ...
            if not df_matchups.empty:
                ...
                df_matchups["my_result_show"] = df_matchups["my_result"].apply(lambda x: "WIN" if x == "victory" else "LOSS")
                df_matchups["opp_result_show"] = df_matchups["opp_result"].apply(lambda x: "WIN" if x == "victory" else "LOSS")

                ev_match = st.dataframe(
                    df_matchups,
                    use_container_width=True,
                    column_order=["game", "battle_time", "map", "mode", "my_result_show", "opp_result_show"],
                    column_config={
                        "game": st.column_config.NumberColumn("Game", format="%d"),
                        "battle_time": st.column_config.TextColumn("Date / Time"),
                        "map": st.column_config.TextColumn("Map"),
...
if not st.session_state.f_team:
    ...
    with tcol1:
        granularity = st.radio("Granularity", options=["Day", "Week"], horizontal=True, key="tl_gran")
    with tcol2:
        wr_type = st.radio("Win Rate", options=["Daily", "Cumulative"], horizontal=True, key="tl_type")
